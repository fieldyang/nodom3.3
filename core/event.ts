...
export class NEvent {
            ...
            if (Util.isArray(arr)) {
                if (arr.length > 0) {
                    for (let i = 0; i < arr.length; i++) {
                        let key = arr[i].dom.key;
                        let el: HTMLElement = <HTMLElement>module.objectManager.getNode(key);
                        if (!el) {
                            continue;
                        }
                        // 找到对应的子事件执行
                        if (eKey === key || el.querySelector(`[key='${key}']`)) {
                            //执行
                            arr[i].fire(e);
                            //执行一次，需要移除
                            if (arr[i].once) {
     ...
     */
    private addChild(ev) {
        let chds = this.getParam('children');
        if(!chds){
            this.setParam('children',[ev]);
        }else{
            chds.push(ev);
        }
    }
